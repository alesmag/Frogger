...
//Librerie per la gestione dei thread, mutex e semafori
#include <semaphore.h>
#include <pthread.h>
...
// Gestione del thread timer
void timer_process();

// Funzione richiamata per il thread timer
void* _timer(void* arg);

// Chiusura del thread timer
void close_timer(object timer);

// Chiude tutti i thread creati
...
// Ricerca uno specifico oggetto contando il numero di oggetti nelle corsie, se non viene trovato restituisce un oggetto resettato
object which_obj(lane_area area, int id, int *line);

// Inserisce un oggetto nel buffer
void push_obj(object obj);

// Rimuove un oggetto dal buffer e lo assegna alla variabile taken_obj
void pop_obj();
...
// Gestisce il thread di una macchina
void car_process();

// Chiude il thread di ogni macchina
void close_cars(object cars[]);

/* Frog */

// Funzione richiamata per il thread rana
void* _frog(void* arg);

// Gestisce il thread della rana
void frog_process();

// Chiude il thread rana
