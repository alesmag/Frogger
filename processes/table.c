...
void draw_high_grass(WINDOW *win, high_grass hg)
{
  ...
  for(i = hg.grass.pos.y; i < hg.grass.pos.y + (hg.grass.height - FROG_HEIGHT); i++)
    for(j = hg.grass.pos.x; j < hg.grass.pos.x + LEN_XT; j++)
      mvwaddch(win, i, j, hg.grass.texture);

  i = 0;

  for(j = hg.grass.pos.x; j < hg.grass.pos.x + LEN_XT; j++)
  {
    if(j == hg.nest[k].pos.x)
    {
      for(g = 0; g < hg.nest[k].len; g++)
      {
        wattrset(win, COLOR_PAIR(BLACK_BLACK));
        mvwaddch(win, hg.grass.pos.y + i + 1, j, hg.nest[k].c);
        j++;
      }
    ...
    {
      wattrset(win, COLOR_PAIR(BLACK_GREEN));
      mvwaddch(win, hg.grass.pos.y + i + 1, j, hg.grass.texture);
      mvwaddch(win, hg.grass.pos.y + i + 2, j, hg.grass.texture);
    }

    if(j == hg.grass.pos.x + LEN_XT)
    {
      i++;
      j = hg.grass.pos.x;
    }

    if(i == hg.grass.height - 1)
      break;
  }
}
