...
extern int num_log, num_car, vite, score, num_enemy, diff, num_laser;
...
void area_gioco(WINDOW *win, int *pipe, table *t)
{
  ...
  while(timer.id > 0)
  {
    read(pipe[READING], &readVal, sizeof(object));

    draw_stats(win);
    draw_frog_in_nest(win, *t);
    ...
    if(readVal.c == FROG_C)
    {
      if((frog.pos.x + (readVal.pos.x - frog_curr_pos.x) <= LEN_XT + START_TX - FROG_LEN) && (frog.pos.x + (readVal.pos.x - frog_curr_pos.x) >= START_TX))
      {
        ...
        if(frog.pos.x >= 0)
          reset_table(win, frog, *t, FROG_HEIGHT);
    ...
    else if(readVal.c == LOG_C)
    {
      if(log[readVal.id].pos.x >= 0)
        reset_table(win, log[readVal.id], *t, FROG_HEIGHT);
    ...
    else if(readVal.c == CAR_C)
    {
      if(car[readVal.id].pos.x >= 0)
        reset_table(win, car[readVal.id], *t, FROG_HEIGHT);
    ...
    else if(readVal.c == BULLET_C)
    {
      if(bullet[readVal.id].pos.x >= 0)
        reset_table(win, bullet[readVal.id], *t, 1);
          
      if(bullet_enemy_collision(enemy, bullet[readVal.id], id_log_full) != NEG_VAL )
      {
        score += ENEMY_SCORE;
        kill(bullet[readVal.id].pid, SIGKILL);
    ...
    else if(readVal.c == LASER_C)
    {
      if(laser[readVal.id].pos.x >= 0)
        reset_table(win, laser[readVal.id], *t, 1);

      else if(laser_bullet_collision(laser[readVal.id], bullet) != NEG_VAL)
      {
        kill(laser[readVal.id].pid, SIGKILL);
    ...
    if(timer.id < temp_timer)
    {
      ...
      if(num_enemy > 0)
      {
        ...
        for(int i = 0; i < num_rand; i++)
          genera_bullet(enemy, pipe[WRITING], id_log_full);
      }
  ...
  // Chiusura dei canali della pipe 
  close(pipe[READING]);
  close(pipe[WRITING]);
}
